 ...
 */
function withPagination({methodName = 'paginate', primaryKeyField = 'id', oneBaseIndex = false, pageSize: __pageSize = 1, where: _where = undefined, orders: _orders = [], attributes: _attributes = undefined, include: _include = undefined, _paranoid=false} = {}) {
    let __pageIndex = 0;
    if(oneBaseIndex) {
        __pageIndex = 1;
    }
    ...
    return model => {
         ...
         */
        const paginate = ({primaryDesc = false, pageSize = __pageSize, pageIndex = __pageIndex, where = _where, orders = _orders, attributes = _attributes, include = _include, paranoid = _paranoid} = {}) => {
            ...
            if (paranoid !== undefined)
                findAllQueryObject.paranoid = paranoid;
            ...
            return model.count({
                ...
                distinct: !!findAllQueryObject.include ? true: false,
                paranoid: findAllQueryObject.paranoid
            })
                .then(size => {
